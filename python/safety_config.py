...
class SafetyConfig:
    ...
    def validate_gps_integrity(cls, gps_data: Dict[str, Any], is_sitl: bool = False) -> tuple[bool, str]:
        ...
        # Check HDOP (GPS accuracy) - more lenient for SITL
        eph = gps_data.get('eph', 999)  # Horizontal accuracy
        
        if is_sitl:
            # SITL GPS is often poor quality but safe for testing
            if eph > 200:  # Very lenient for SITL
                return False, f"GPS accuracy extremely poor even for SITL: HDOP {eph} (need <200)"
            elif eph > 50:
                print(f"⚠️ SITL GPS accuracy poor but acceptable: HDOP {eph}")
        else:
